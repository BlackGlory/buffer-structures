...
举例来说, 由于此库提供的HashMap比原生Map慢几个数量级, 任何大量访问HashMap的项目都会因此出现严重的性能问题.
...
对于此库中的数据结构来说, 如果相关的缓冲区空间能够随着垃圾回收而自动释放, 那会是一件很酷的事.
...
### 与类型化数组的区别是什么?
与类型化数组等数据结构不同, 此库中的数据结构将缓冲区视作一种可分配和释放的线性内存来使用.
