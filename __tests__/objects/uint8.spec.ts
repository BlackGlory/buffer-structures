...
describe('Uint8', () => {
  test('create', () => {
    ...
    const result = Uint8.create(allocator, uint8(1))

    expect(result).toBeInstanceOf(BaseObject)
    expect(result.get()).toStrictEqual(uint8(1))
    expect(allocate).toBeCalledTimes(1)
    expect(allocate).toBeCalledWith(Uint8View.byteLength)
  })

  test('from', () => {
    const allocator = new Allocator(new ArrayBuffer(100))
    const obj = Uint8.create(allocator, uint8(1))
    const allocate = jest.spyOn(allocator, 'allocate')

    const result = Uint8.from(allocator, obj.byteOffset)
  ...
  describe('destory', () => {
    it('calls allocator.free()', () => {
      ...
      expect(free).toBeCalledWith(result._view.byteOffset, Uint8View.byteLength)
    })
    ...
    describe('reference counted', () => {
      test('does not call allocator.free()', () => {
        ...
        const obj1 = Uint8.create(allocator, uint8(1))
      ...
      test('calls allocator.free()', () => {
        ...
        expect(free).toBeCalledTimes(1)
        expect(free).toBeCalledWith(obj1._view.byteOffset, Uint8View.byteLength)
      })
    })
  })
  ...
  test('set', () => {
    ...
    expect(obj.get()).toStrictEqual(uint8(2))
  })
